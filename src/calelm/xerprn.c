/* xerprn.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
int
nsp_calpack_xerprn (char *prefix, int *npref, char *messg, int *nwrap,
		    long int prefix_len, long int messg_len)
{
  ...
  int i_indx (char *, char *, long int, long int), s_cmp (char *, char *,
							  long int, long int);

  /* Local variables */
  int i__, n;
  char cbuff[148];
  int lpref, nextc, lwrap, nunit;
  int io, iu[5], lpiece, idelta, lenmsg;
  double wte;
   ...
   *int units to which error messages are to be sent.  This routine 
   ...
   *        PREFIX will be used. 
   * 
   *NPREF   Input argument of type INT.  This argument is the number 
   ...
   * 
   *NWRAP   Input argument of type INT.  This gives the maximum size 
   ...
   */
  nsp_calpack_xgetua (iu, &nunit);
  /* 
   *      A ZERO VALUE FOR A INT UNIT NUMBER MEANS TO USE THE STANDARD 
   *      ERROR MESSAGE UNIT INSTEAD.  I1MACH(4) RETRIEVES THE STANDARD 
   *      ERROR MESSAGE UNIT. 
   * 
   */
  n = nsp_calpack_i1mach (&c__4);
  i__1 = nunit;
  ...
  if (lenmsg == 0)
    {
      ...
      /*    THREE NEXT LINES REPLACED FOR SCILAB INTERFACE 
       *STD         DO 40 I=1,NUNIT 
       *STD            WRITE(IU(I), '(A)') CBUFF(1:LPREF+1) 
       *STD   40    CONTINUE 
       */
      nsp_calpack_basout (&io, &wte, cbuff, lpref + 1);
      return 0;
    }
  /* 
...
L50:
   ...
   *    THREE NEXT LINES REPLACED FOR SCILAB INTERFACE 
   *STD       DO 60 I=1,NUNIT 
   *STD          WRITE(IU(I), '(A)') CBUFF(1:LPREF+LPIECE) 
   *STD    60 CONTINUE 
   */
  nsp_calpack_basout (&io, &wte, cbuff, lpref + lpiece);
  /* 
   */
  if (nextc <= lenmsg)
    {
