/* zunk1.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
int
nsp_calpack_zunk1 (double *zr, double *zi, double *fnu, int *kode, int *mr,
		   ...
		   double *elim, double *alim)
{
  ...
  /* Local variables */
  double aphi, cscl, phii[2], crsc;
  double phir[2];
  int init[2];
  double csrr[3], cssr[3], rast, sumi[2], razr;
  double sumr[2];
  int i__, j, k, m, iflag, kflag;
  double ascle;
  int kdflg;
  double phidi;
  int ipard;
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      ...
      fn = *fnu + (i__ - 1);
      init[j - 1] = 0;
      nsp_calpack_zunik (&zrr, &zri, &fn, &c__2, &c__0, tol, &init[j - 1],
...
L75:
  ...
  initd = 0;
  nsp_calpack_zunik (&zrr, &zri, &fn, &c__2, &ipard, tol, &initd, &phidr,
		     &phidi, &zet1dr, &zet1di, &zet2dr, &zet2di, &sumdr,
		     &sumdi, &cwrkr[32], &cwrki[32]);
  if (*kode == 1)
    {
      goto L80;
    }
...
L170:
  ...
  for (k = 1; k <= i__1; ++k)
      ...
      if (kk == ib || kk == ic)
	{
      ...
      initd = 0;
    L180:
      nsp_calpack_zunik (&zrr, &zri, &fn, &c__1, &c__0, tol, &initd, &phidr,
			 &phidi, &zet1dr, &zet1di, &zet2dr, &zet2di, &sumdr,
			 &sumdi, &cwrkr[(m << 4) - 16],
			 &cwrki[(m << 4) - 16]);
      if (*kode == 1)
