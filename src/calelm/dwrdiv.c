/* dwrdiv.f -- translated by f2c (version 19961017).
 *
 *
 */
...
int
nsp_calpack_dwrdiv (double *ar, int *ia, double *br, double *bi, int *ib,
		    double *rr, double *ri, int *ir, int *n, int *ierr)
{
  ...
  /*!    purpose 
   *    computes r=a./b with a complex vector and b real vector 
   * 
   *    ia,ib,ir : increment between two consecutive element of vectors a 
   *               b and r 
   *    ar       : array containing a 
  ...
  if (*ia == 0)
    {
      ...
      for (k = 1; k <= i__1; ++k)
	{
	  nsp_calpack_dwdiv (&ar[ja], &br[jb], &bi[jb], &wr, &wi, &ierr1);
	  rr[jr] = wr;
	  ri[jr] = wi;
	  if (ierr1 != 0)
	    {
