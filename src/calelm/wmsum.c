/* wmsum.f -- translated by f2c (version 19961017).
 *
 *
 */
...
int
nsp_calpack_wmsum (int *flag__, double *ar, double *ai, int *na, int *m,
		   int *n, double *vr, double *vi, int *nv)
{
  ...
  /*!purpose 
   *    computes the sum of the entries of a complexmatrix according to flag 
   *!calling sequence 
  ...
  if (*flag__ == 0)
    {
      ...
      for (j = 1; j <= i__1; ++j)
	{
	  tr += nsp_calpack_dsum (m, &ar[j * ar_dim1 + 1], &c__1);
	  ti += nsp_calpack_dsum (m, &ai[j * ai_dim1 + 1], &c__1);
	  /* L10: */
	}
      vr[1] = tr;
      vi[1] = ti;
	...
	{
	  tr = nsp_calpack_dsum (n, &ar[i__ + ar_dim1], m);
	  ti = nsp_calpack_dsum (n, &ai[i__ + ai_dim1], m);
	  vr[iv] = tr;
	  vi[iv] = ti;
	  iv += *nv;
	  /* L30: */
