/* zbesh.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
int
nsp_calpack_zbesh (double *zr, double *zi, double *fnu, int *kode, int *m,
		   int *n, double *cyr, double *cyi, int *nz, int *ierr)
{
  ...
  *nz = 0;
   ...
   *----------------------------------------------------------------------- 
   */
  ufl = nsp_calpack_d1mach (&c__1) * 1e3;
  if (az < ufl)
    {
      goto L230;
    }
  ...
  goto L70;
L60:
  nsp_calpack_zuoik (&znr, &zni, fnu, kode, &c__2, &nn, &cyr[1], &cyi[1],
		     &nuf, &tol, &elim, &alim);
  if (nuf < 0)
    {
      goto L230;
...
L70:
   ...
   *----------------------------------------------------------------------- 
   */
  nsp_calpack_zbknu (&znr, &zni, fnu, kode, &nn, &cyr[1], &cyi[1], nz, &tol,
		     &elim, &alim);
  goto L110;
  /*----------------------------------------------------------------------- 
...
L80:
  mr = -mm;
  nsp_calpack_zacon (&znr, &zni, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw,
...
L90:
  ...
  zni = -zni;
L100:
  nsp_calpack_zbunk (&znr, &zni, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw,
		     &tol, &elim, &alim);
  if (nw < 0)
    {
      goto L240;
