/* zunk2.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
int
nsp_calpack_zunk2 (double *zr, double *zi, double *fnu, int *kode, int *mr,
...
L20:
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      ...
      j = 3 - j;
      fn = *fnu + (i__ - 1);
      nsp_calpack_zunhj (&znr, &zni, &fn, &c__0, tol, &phir[j - 1],
			 &phii[j - 1], &argr[j - 1], &argi[j - 1],
			 &zeta1r[j - 1], &zeta1i[j - 1], &zeta2r[j - 1],
			 &zeta2i[j - 1], &asumr[j - 1], &asumi[j - 1],
			 &bsumr[j - 1], &bsumi[j - 1]);
      if (*kode == 1)
	{
	  goto L30;
      ...
      if (Abs (rs1) < *alim)
	{
       ...
       *----------------------------------------------------------------------- 
       */
      aphi = nsp_calpack_zabs (&phir[j - 1], &phii[j - 1]);
      aarg = nsp_calpack_zabs (&argr[j - 1], &argi[j - 1]);
    ...
    L50:
      ...
      nsp_calpack_zairy (&c2r, &c2i, &c__0, &c__2, &air, &aii, &nai, &idum);
      nsp_calpack_zairy (&c2r, &c2i, &c__1, &c__2, &dair, &daii, &ndai,
			 &idum);
  ...
  i__ = *n;
L85:
  razr = 1. / nsp_calpack_zabs (&zrr, &zri);
  str = zrr * razr;
  sti = -zri * razr;
  rzr = (str + str) * razr;
  rzi = (sti + sti) * razr;
  ...
  nsp_calpack_zunhj (&znr, &zni, &fn, &ipard, tol, &phidr, &phidi, &argdr,
...
L100:
   ...
   *----------------------------------------------------------------------- 
   */
  aphi = nsp_calpack_zabs (&phidr, &phidi);
  rs1 += log (aphi);
  if (Abs (rs1) < *elim)
    {
      goto L120;
...
L190:
  ...
  for (k = 1; k <= i__1; ++k)
      ...
      if (kk == ib || kk == ic)
	{
      ...
      nsp_calpack_zunhj (&znr, &zni, &fn, &c__0, tol, &phidr, &phidi, &argdr,
			 &argdi, &zet1dr, &zet1di, &zet2dr, &zet2di, &asumdr,
			 &asumdi, &bsumdr, &bsumdi);
    L210:
      if (*kode == 1)
      ...
      if (Abs (rs1) < *alim)
	{
       ...
       */
      aphi = nsp_calpack_zabs (&phidr, &phidi);
      aarg = nsp_calpack_zabs (&argdr, &argdi);
      rs1 = rs1 + log (aphi) - log (aarg) * .25 - aic;
      if (Abs (rs1) > *elim)
	{
	  goto L280;
    ...
    L240:
      nsp_calpack_zairy (&argdr, &argdi, &c__0, &c__2, &air, &aii, &nai,
			 &idum);
      nsp_calpack_zairy (&argdr, &argdi, &c__1, &c__2, &dair, &daii, &ndai,
			 &idum);
      ...
      if (*kode == 1)
	{
      ...
      nsp_calpack_zs1s2 (&zrr, &zri, &s1r, &s1i, &s2r, &s2i, &nw, &asc, alim,
			 &iuf);
      *nz += nw;
    L270:
...
L295:
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
      ...
      if (*kode == 1)
	{
      ...
      nsp_calpack_zs1s2 (&zrr, &zri, &c1r, &c1i, &c2r, &c2i, &nw, &asc, alim,
			 &iuf);
      *nz += nw;
    L300:
