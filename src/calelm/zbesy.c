/* zbesy.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
int
nsp_calpack_zbesy (double *zr, double *zi, double *fnu, int *kode, int *n,
		   ...
		   double *cwrki, int *ierr)
{
  ...
  *nz = 0;
  ...
  hcii = .5;
  nsp_calpack_zbesh (zr, zi, fnu, kode, &c__1, n, &cyr[1], &cyi[1], &nz1,
		     ierr);
  if (*ierr != 0 && *ierr != 3)
    {
      goto L170;
    }
  nsp_calpack_zbesh (zr, zi, fnu, kode, &c__2, n, &cwrkr[1], &cwrki[1], &nz2,
		     ierr);
  if (*ierr != 0 && *ierr != 3)
    {
      goto L170;
