/* rjbesl.f -- translated by f2c (version 19961017).
 *
 *
 */
...
int
nsp_calpack_rjbesl (double *x, double *alpha, int *nb, double *b, int *ncalc)
{
  /* Initialized data */

  static double pi2 = .636619772367581343075535;
  static double four = 4.;
  static double twofiv = 25.;
  static double one30 = 130.;
  static double three5 = 35.;
  ...
  static double fact[25] =
    ...
    39916800., 479001600., 6227020800., 87178291200., 1.307674368e12,
    2.0922789888e13,
  ...
  /* Builtin functions */
  double pow_dd (double *, double *), sqrt (double), d_int (double *),
  ...
  double pold;
  int nbmx;
  double vcos, test, vsin;
  int i__, j, k, l, m, n;
  double p, s, t, z__, alpem, halfx, tempa, tempb, tempc, psave, plast, tover,
    t1, alp2em, em, en;
  double xc, xk, xm, psavel;
  int nstart;
  double gnu, xin, sum;
   ...
   *  NB  - int number of functions to be calculated, NB > 0. 
   ...
   *  argument and int order.  Modifications include the restriction 
   ...
   *              pp 941-947. 
   * 
   *             "Bessel Functions of Real Argument and Int Order," 
   ...
   */
  magx = (int) (*x);
  if (*nb > 0 && *x >= zero && *x <= xlarge && *alpha >= zero && *alpha < one)
    {
      /*--------------------------------------------------------------------- 
	  ...
	  if (*alpha != zero)
	    {
	      tempa =
		pow_dd (&halfx,
			alpha) / (*alpha * nsp_calpack_dgamma (alpha));
	    }
	  tempb = zero;
	  if (*x + one > one)
	    {
	  ...
	  if (nbmx >= 3)
	    {
	      ...
	      nend = *nb - 1;
	      i__1 = nstart + nstart;
	      en = (double) i__1 - two + (*alpha + *alpha);
	      i__1 = nend;
	  ...
	  if (*alpha + one != one)
	    {
	      d__1 = *x * half;
	      d__2 = -(*alpha);
	      sum = sum * nsp_calpack_dgamma (alpha) * pow (d__1, d__2);
	    }
	  tempa = enmten;
	  if (sum > one)
	    {
