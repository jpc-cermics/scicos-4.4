/* zwrsk.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
int
nsp_calpack_zwrsk (double *zrr, double *zri, double *fnu, int *kode, int *n,
		   ...
		   double *tol, double *elim, double *alim)
{
  ...
  /* Function Body */
  *nz = 0;
  nsp_calpack_zbknu (zrr, zri, fnu, kode, &c__2, &cwr[1], &cwi[1], &nw, tol,
		     elim, alim);
  if (nw != 0)
    {
      goto L50;
    }
  nsp_calpack_zrati (zrr, zri, fnu, n, &yr[1], &yi[1], tol);
  /*----------------------------------------------------------------------- 
...
L10:
   ...
   *----------------------------------------------------------------------- 
   */
  acw = nsp_calpack_zabs (&cwr[2], &cwi[2]);
