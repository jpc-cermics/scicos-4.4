/* wwpow1.f -- translated by f2c (version 19961017).
 *
 *
 */
...
int
nsp_calpack_wwpow1 (int *n, double *vr, double *vi, int *iv, double *pr,
		    double *pi, int *ip, double *rr, double *ri, int *ir,
		    int *ierr)
{
  ...
  /*!purpose 
   *    computes V^P with V  and P complex vectors 
   *!calling sequence 
  ...
  *ierr = 0;
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
    {
      nsp_calpack_wwpowe (&vr[ii], &vi[ii], &pr[iip], &pi[iip], &rr[iir],
			  &ri[iir], &ierr1);
      *ierr = Max (*ierr, ierr1);
      /*        if(ierr.ne.0) return 
       */
