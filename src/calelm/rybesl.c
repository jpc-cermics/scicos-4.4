/* rybesl.f -- translated by f2c (version 19961017).
 *
 *
 */
...
int
nsp_calpack_rybesl (double *x, double *alpha, int *nb, double *by, int *ncalc)
{
  /* Initialized data */

  static double zero = 0.;
  static double piby2 = 1.5707963267948966192;
  static double pi = 3.1415926535897932385;
  static double sq2bpi = .79788456080286535588;
  static double pim5 = .70796326794896619231;
  static double onbpi = .31830988618379067154;
  static double del = 1e-8;
  ...
  static double ch[21] =
    { -6.7735241822398840964e-24, -6.1455180116049879894e-23,
    2.9017595056104745456e-21, 1.3639417919073099464e-19,
    ...
    -4.9717367041957398581e-6, 7.6309597585908126618e-5,
    .0012719271366545622927,
    .0017063050710955562222, -.07685284084478667369, -.28387654227602353814,
    .92187029365045265648
  };
  static double one = 1.;
  static double two = 2.;
  ...
  /* Builtin functions */
  double d_int (double *), sqrt (double), sin (double), cos (double),
    log (double), pow_dd (double *, double *);

  /* Local variables */
  double alfa, ddiv, even, term, b, c__, d__, e, f, g, h__;
  int i__, k;
  double p, gamma, q, r__, s, d1, cosmu, d2, sinmu, q0, x2;
  int na;
  double pa, qa, en, ya, ex, pa1, qa1, en1, ya1, twobyx, den, odd, aye, div,
    dmu, xna, enu;
  ...
  if (*nb > 0 && *x >= xmin && ex < xlarge && enu >= zero && enu < one)
	...
	L100:
	  en += one;
	  if ((d__1 = g / (one + Abs (ya)), Abs (d__1)) + (d__2 =
							   h__ / (one +
								  Abs (ya1)),
							   Abs (d__2)) > eps)
	    {
	  ...
	  for (i__ = 3; i__ <= i__1; ++i__)
	    {
	      if (twobyx < one)
		{
		  if ((d__1 = by[i__ - 1], Abs (d__1)) * twobyx >= xinf / aye)
		    {
		      goto L450;
		    }
		}
		...
		{
		  if ((d__1 = by[i__ - 1], Abs (d__1)) >= xinf / aye / twobyx)
		    {
		      goto L450;
		    }
		}
