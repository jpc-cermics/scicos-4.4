/* gamma.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
double nsp_calpack_dgammacody (double *x)
{
  /* Initialized data */
  ...
  /* Builtin functions */
  double d_int (double *), sin (double), log (double), exp (double);
   ...
   *        2/ when x is a negative int return Nan (in place of XINF) 
   ...
   *      exception may be returned (with an int flag say IERR) in theses 
   ...
   *---------------------------------------------------------------------- 
   * Statement functions for conversion between int and float 
   *---------------------------------------------------------------------- 
   *S    CONV(I) = REAL(I) 
   */
  parity = FALSE;
  ...
  if (Abs (y) < eps)
	...
	{
	  /*         RES = XINF 
	   *modif Bruno: return Nan (Y is a negative int) 
	   */
	  res = return_a_nan_nsp_calpack_ ();
	  /*this one is defined in somespline.f 
	   */
	  goto L900;
	}
    }
