/* zbknu.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
int
nsp_calpack_zbknu (double *zr, double *zi, double *fnu, int *kode, int *n,
		   ...
		   double *elim, double *alim)
{
  ...
  *nz = 0;
   ...
   */
  fc = 1.;
  nsp_calpack_zlog (&rzr, &rzi, &smur, &smui, &idum);
  fmur = smur * dnu;
  fmui = smui * dnu;
  nsp_calpack_zshch (&fmur, &fmui, &cshr, &cshi, &cchr, &cchi);
  if (dnu == 0.)
    {
      goto L10;
    }
...
L10:
   ...
   *----------------------------------------------------------------------- 
   */
  t2 = exp (-nsp_calpack_dgamln (&a2, &idum));
  t1 = 1. / (t2 * fc);
  if (Abs (dnu) > .1)
    {
      goto L40;
...
L50:
  ...
  nsp_calpack_zexp (&fmur, &fmui, &str, &sti);
  pr = str * .5 / t2;
  pi = sti * .5 / t2;
  nsp_calpack_zdiv (&c_b10, &c_b11, &str, &sti, &ptr, &pti);
  qr = ptr / t1;
  qi = pti / t1;
  s1r = fr;
  s1i = fi;
  ...
  nsp_calpack_zmlt (zr, zi, zr, zi, &czr, &czi);
  czr *= .25;
  czi *= .25;
  t1 = caz * .25 * caz;
L60:
...
L70:
  ...
  nsp_calpack_zexp (zr, zi, &str, &sti);
  nsp_calpack_zmlt (&s1r, &s1i, &str, &sti, &yr[1], &yi[1]);
  return 0;
  /*----------------------------------------------------------------------- 
...
L100:
  ...
  p2r = s2r * str;
  p2i = s2i * str;
  nsp_calpack_zmlt (&p2r, &p2i, &rzr, &rzi, &s2r, &s2i);
  s1r *= str;
  s1i *= str;
  if (koded == 1)
    {
  ...
  nsp_calpack_zexp (zr, zi, &fr, &fi);
  nsp_calpack_zmlt (&s1r, &s1i, &fr, &fi, &s1r, &s1i);
  nsp_calpack_zmlt (&s2r, &s2i, &fr, &fi, &s2r, &s2i);
  goto L210;
  /*----------------------------------------------------------------------- 
   ...
   */
L110:
  nsp_calpack_zsqrt (zr, zi, &str, &sti);
  nsp_calpack_zdiv (&rthpi, &czeroi, &str, &sti, &coefr, &coefi);
  kflag = 2;
  if (koded == 2)
    {
      goto L120;
  ...
  sti = -str * sin (*zi);
  str *= cos (*zi);
  nsp_calpack_zmlt (&coefr, &coefi, &str, &sti, &coefr, &coefi);
L120:
  if (Abs (dnu) == .5)
    {
      goto L300;
   ...
   *----------------------------------------------------------------------- 
   */
  t1 = (double) (nsp_calpack_i1mach (&c__14) - 1);
  t1 = t1 * nsp_calpack_d1mach (&c__5) * 3.321928094;
  t1 = Max (t1, 12.);
  t1 = Min (t1, 60.);
  t2 = tth * t1 - 6.;
  if (*zr != 0.)
...
L180:
   ...
   *----------------------------------------------------------------------- 
   */
  tm = nsp_calpack_zabs (&csr, &csi);
  ptr = 1. / tm;
  s1r = p2r * ptr;
  s1i = p2i * ptr;
  csr *= ptr;
  csi = -csi * ptr;
  nsp_calpack_zmlt (&coefr, &coefi, &s1r, &s1i, &str, &sti);
  nsp_calpack_zmlt (&str, &sti, &csr, &csi, &s1r, &s1i);
  if (inu > 0 || *n > 1)
    {
      goto L200;
    }
...
L200:
   ...
   *----------------------------------------------------------------------- 
   */
  tm = nsp_calpack_zabs (&p2r, &p2i);
  ptr = 1. / tm;
  p1r *= ptr;
  p1i *= ptr;
  p2r *= ptr;
  p2i = -p2i * ptr;
  nsp_calpack_zmlt (&p1r, &p1i, &p2r, &p2i, &ptr, &pti);
  str = dnu + .5 - ptr;
  sti = -pti;
  nsp_calpack_zdiv (&str, &sti, zr, zi, &str, &sti);
  str += 1.;
  nsp_calpack_zmlt (&str, &sti, &s1r, &s1i, &s2r, &s2i);
  /*----------------------------------------------------------------------- 
...
L261:
  ...
  for (i__ = 1; i__ <= i__1; ++i__)
      ...
      if (p2r < -(*elim))
	{
      ...
      nsp_calpack_zlog (&s2r, &s2i, &str, &sti, &idum);
      p2r = -zdr + str;
      p2i = -zdi + sti;
      p2m = exp (p2r) / *tol;
      p1r = p2m * cos (p2i);
      p1i = p2m * sin (p2i);
      nsp_calpack_zuchk (&p1r, &p1i, &nw, &ascle, tol);
      if (nw != 0)
	{
	  goto L263;
	}
...
L280:
  ascle = bry[0];
  nsp_calpack_zkscl (&zdr, &zdi, fnu, n, &yr[1], &yi[1], nz, &rzr, &rzi,
		     &ascle, tol, elim);
  inu = *n - *nz;
  if (inu <= 0)
    {
