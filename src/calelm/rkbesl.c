/* rkbesl.f -- translated by f2c (version 19961017).
 *
 *
 */
...
int
nsp_calpack_rkbesl (double *x, double *alpha, int *nb, int *ize, double *bk,
		    int *ncalc)
{
  /* Initialized data */
  ...
  static double p[8] =
    { .805629875690432845, 20.4045500205365151, 157.705605106676174,
    536.671116469207504, 900.382759291288778, 730.923886650660393,
    229.299301509425145,
    .822467033424113231
  };
  static double q[7] =
    { 29.4601986247850434, 277.577868510221208, 1206.70325591027438,
    2762.91444159791519, 3443.74050506564618, 2210.63190113378647,
    572.267338359892221
  };
  static double r__[5] =
    { -.48672575865218401848, 13.079485869097804016, -101.96490580880537526,
    347.65409106507813131, 3.495898124521934782e-4
  };
  static double s[4] =
    { -25.579105509976461286, 212.57260432226544008, -610.69018684944109624,
    422.69668805777760407
  };
  static double t[6] = { 1.6125990452916363814e-10, 2.5051878502858255354e-8,
    2.7557319615147964774e-6, 1.9841269840928373686e-4,
    .0083333333333334751799,
    .16666666666666666446
  };
  static double estm[6] =
    { 52.0583, 5.7607, 2.7782, 14.4303, 185.3004, 9.3715 };
  static double one = 1.;
  static double estf[7] =
  ...
  static double d__ = .797884560802865364;
  ...
  /* Builtin functions */
  double log (double), exp (double), sinh (double), sqrt (double),
    d_int (double *);

  /* Local variables */
  int iend;
  double x2by4, twox, c__;
  int i__, j, k, m;
  double blpha;
  int itemp;
   ...
   *NB    - Int number of functions to be calculated, NB .GT. 0. 
   *        The first function calculated is of order ALPHA, and the 
   *        last is of order (NB - 1 + ALPHA). 
   *IZE   - Int type.  IZE = 1 if unscaled K's are to be calculated, 
   ...
   *NCALC - Int output variable indicating possible errors. 
      ...
      if (ex <= one)
	{
	  ...
	  if (ex <= tinyx)
	    ...
	    L100:
	      ...
	      bk1 += t1;
	      bk2 += t2;
	      if ((d__1 = t1 / (f1 + bk1), Abs (d__1)) > eps
		  || (d__2 = t2 / (f2 + bk2), Abs (d__2)) > eps)
		{
		  goto L100;
		}
	      bk1 = f1 + bk1;
      ...
      else
	{
	  ...
	  if (ex <= four)
	      ...
	      for (i__ = 2; i__ <= i__1; ++i__)
		{
	      ...
	      f1 = xmin;
	      f0 =
		(two * (c__ + d2) / ex + half * ex / (c__ + d2 + one)) * xmin;
	      i__1 = m;
	      ...
	      for (i__ = 1; i__ <= 7; ++i__)
		{
	      ...
	      p0 =
		exp (c__ * (a + c__ * (p[7] - c__ * d1 / t1) - log (ex))) /
		ex;
	      ...
	      if (*ize == 1)
		{
	      ...
	      wminf =
		estf[4] * (ex - (d__1 = ex - estf[6], Abs (d__1))) + estf[5];
	    }
	  /*-------------------------------------------------------------------- 
