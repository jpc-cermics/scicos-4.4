/* zbesjg.f -- translated by f2c (version 19961017).
 *
 *
 */
...
int
nsp_calpack_zbesjg (double *x1r, double *x1i, double *alpha, int *kode,
		    ...
		    double *wi, int *ierr)
{
   ...
   *    extends cbesj for the case where alpha is negative 
   * 
   */
  ...
  if (xr != xr || xi != xi || *alpha != *alpha)
    ...
    {
      /*    .  alpha >= 0 the simple case CHECKED 
       */
      nsp_calpack_zbesj (&xr, &xi, alpha, kode, n, &yr[1], &yi[1], nz, ierr);
      if (*ierr == 2)
	{
      ...
      else
	{
      ...
      nsp_calpack_zbesj (&xr, &xi, &a1, kode, n, &wr[1], &wi[1], nz, ierr);
      if (*ierr == 2)
	{
	  nsp_calpack_dset (n, &inf, &yr[1], &c__1);
	  nsp_calpack_dset (n, &inf, &yi[1], &c__1);
      ...
      else
	{
      ...
      i__1 = (nn - i0 + 1) / 2;
      C2F (dscal) (&i__1, &c_b24, &yr[i0 + 1], &c__2);
      i__1 = (nn - i0 + 1) / 2;
      C2F (dscal) (&i__1, &c_b24, &yi[i0 + 1], &c__2);
    }
  else if (xr == 0. && xi == 0.)
    {
      d__1 = -inf;
      nsp_calpack_dset (n, &d__1, &yr[1], &c__1);
      nsp_calpack_dset (n, &c_b29, &yi[1], &c__1);
      *ierr = 2;
    }
  else
    {
      /*    .  alpha is negative non int CHECKED 
       *    .  transform to positive value of alpha 
       */
      if (*alpha - 1. + *n >= 0.)
	{
      ...
      else if (*ierr == 4)
	{
       ...
       *    .  sin((a1+k)*pi) change sign with k 
       */
      if (nn >= 2)
	{
	  i__1 = nn / 2;
	  C2F (dscal) (&i__1, &c_b24, &wr[2], &c__2);
	  i__1 = nn / 2;
	  C2F (dscal) (&i__1, &c_b24, &wi[2], &c__2);
	}
      ...
      if (*n > nn)
	{
	  ...
	  a1 = 1. - a1;
	  i__1 = *n - nn;
	  nsp_calpack_zbesj (&xr, &xi, &a1, kode, &i__1, &yr[nn + 1],
			     &yi[nn + 1], nz, &ier);
	  if (ier == 2)
	    {
	      i__1 = *n - nn;
...
int
nsp_calpack_zbesjv (double *xr, double *xi, int *nx, double *alpha, int *na,
		    ...
		    double *wi, int *ierr)
{
  ...
  *ierr = 0;
  ...
  if (*na < 0)
	...
	{
	  nsp_calpack_zbesjg (&xr[i__], &xi[i__], &alpha[i__], kode, &c__1,
  ...
  else if (*na == 1)
    {
      ...
      for (i__ = 1; i__ <= i__1; ++i__)
	{
	  nsp_calpack_zbesjg (&xr[i__], &xi[i__], &alpha[1], kode, &c__1,
			      &yr[i__], &yi[i__], &nz, &wr[1], &wi[1], &ier);
	  *ierr = Max (*ierr, ier);
	}
    }
      ...
      if (j <= *na && (d__1 = alpha[j - 1] + 1 - alpha[j], Abs (d__1)) <= eps)
	{
      ...
      for (i__ = 1; i__ <= i__1; ++i__)
	{
	  nsp_calpack_zbesjg (&xr[i__], &xi[i__], &alpha[j0], kode, &n,
