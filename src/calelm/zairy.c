/* zairy.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
int
nsp_calpack_zairy (double *zr, double *zi, int *id, int *kode, double *air,
		   double *aii, int *nz, int *ierr)
{
  ...
  /* Local variables */
  double sfac, alim, elim, alaz, csqi;
...
L40:
  ...
  *aii = s1i * c1 - c2 * (*zr * s2i + *zi * s2r);
  ...
  nsp_calpack_zexp (&ztar, &ztai, &str, &sti);
  ptr = *air * str - *aii * sti;
  *aii = *air * sti + *aii * str;
  *air = ptr;
  return 0;
...
L70:
  ...
  nsp_calpack_zsqrt (zr, zi, &csqr, &csqi);
...
L100:
  ...
  nsp_calpack_zacai (&ztar, &ztai, &fnu, kode, &mr, &c__1, cyr, cyi, &nn,
		     &rl, &tol, &elim, &alim);
  if (nn < 0)
    {
      goto L280;
...
L120:
  nsp_calpack_zbknu (&ztar, &ztai, &fnu, kode, &c__1, cyr, cyi, nz, &tol,
		     &elim, &alim);
L130:
  s1r = cyr[0] * coef;
  s1i = cyi[0] * coef;
...
L160:
  ...
  *aii = s1i / sfac;
  return 0;
L170:
  aa = nsp_calpack_d1mach (&c__1) * 1e3;
  s1r = zeror;
  s1i = zeroi;
  if (*id == 1)
    {
