/* zbesk.f -- translated by f2c (version 19961017).
 *
 *
 */
 ...
 */
int
nsp_calpack_zbesk (double *zr, double *zi, double *fnu, int *kode, int *n,
		   double *cyr, double *cyi, int *nz, int *ierr)
{
  ...
  *nz = 0;
  ...
  goto L60;
L50:
  nsp_calpack_zuoik (zr, zi, fnu, kode, &c__2, &nn, &cyr[1], &cyi[1], &nuf,
		     &tol, &elim, &alim);
  if (nuf < 0)
    {
      goto L180;
...
L60:
   ...
   *----------------------------------------------------------------------- 
   */
  nsp_calpack_zbknu (zr, zi, fnu, kode, &nn, &cyr[1], &cyi[1], &nw, &tol,
		     &elim, &alim);
  if (nw < 0)
    {
      goto L200;
...
L70:
  ...
  nsp_calpack_zacon (zr, zi, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw, &rl,
		     &fnul, &tol, &elim, &alim);
  if (nw < 0)
    {
      goto L200;
...
L90:
  nsp_calpack_zbunk (zr, zi, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw,
		     &tol, &elim, &alim);
  if (nw < 0)
    {
      goto L200;
