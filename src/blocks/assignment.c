...
void assignment (scicos_block * block, int flag)
{
  ...
  if (flag == 4)
	  ...
	  if ((typ == SCSREAL_N) | (typ == SCSCOMPLEX_N))
	    {
	      ...
	      *tempind1 = 1;
	      *(tempind1 + 1) =
		max (min ((int) (*ind1), my - (1 - (int) (*indxb))) -
		     (int) (*indxb), 0);
	    }
      ...
      else if ((*indxopt == 2) | (*indxopt == 4))
	{
	  *tempind1 = GetOparSize (block, 3, 1) * GetOparSize (block, 3, 2);
	  for (i = 0; i < *tempind1; i++)
	    *(tempind1 + 1 + i) =
	      max (min ((int) (*(indx1 + i)), my - (1 - (int) (*indxb))) -
		   (int) (*indxb), 0);
	}
      else
	{
	...
	{			/*matrix case */
	  if (((*(indxopt + 1) == 3) | (*(indxopt + 1) == 5)) & ((*indxopt ==
								  3) |
								 (*indxopt ==
								  5)))
	    {
	      if ((typ == SCSREAL_N) | (typ == SCSCOMPLEX_N))
		{
		  ...
		  *tempind2 = 1;
		  *(tempind2 + 1) =
		    max (min ((int) (*ind2), ny - (1 - (int) (*indxb))) -
			 (int) (*indxb), 0);
		}
	      else if ((typ == SCSINT8_N) | (typ == SCSUINT8_N))
		{
	  ...
	  else if (((*(indxopt + 1) == 3) | (*(indxopt + 1) == 5)) &
		   (!((*indxopt == 3) | (*indxopt == 5))))
	    {
	      if ((typ == SCSREAL_N) | (typ == SCSCOMPLEX_N))
		{
	    ...
	    {
	      *tempind2 =
		GetOparSize (block, 4, 1) * GetOparSize (block, 4, 2);
	      for (i = 0; i < *tempind2; i++)
		*(tempind2 + 1 + i) =
		  max (min ((int) (*(indx2 + i)), ny - (1 - (int) (*indxb))) -
		       (int) (*indxb), 0);
	    }
	  else
	    {
