...
int scicos_evscpe_block (scicos_args_F0)
{
  ...
  if (*flag__ == 2)
      ...
      while (1)
	{
	  if ((*nevprt & *nx) != 0)
	    {
	      Xgc->graphic_engine->scale->drawpolylines (Xgc, xx, yy,
							 &ipar[i__ + 2], c__1,
							 c__2);
	      yy[0] = (yy[0] + yy[1]) / 2;
	    }
      ...
      if (ipar[iwd] >= 0)
	{
      ...
      nsp_plot2d_old (Xgc, rect, &rect[1], &c__1, &c__1, &c_n1, "011",
		      "t@ @input and output", 0, rect, nax);
      str = scicos_get_label (Scicos->params.curblk);
  ...
  if (*flag__ == 2)
      ...
      if (ipar[6] == 0)
	{
      ...
      */
      ...
      if (ipar[4] < 0)
	{
	  Xgc->graphic_engine->scale->drawpolylines (Xgc, &z__[n + 1],
						     &z__[(n << 1) + 1],
						     &ipar[4], c__1, c__1);
	}
      else
	{
	  Xgc->graphic_engine->scale->drawpolylines (Xgc, &z__[n],
						     &z__[n * 3], &ipar[4],
						     c__1, c__2);
	}
