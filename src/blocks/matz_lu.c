...
void matz_lu (scicos_block * block, int flag)
{
  ...
  if (flag == 4)
    {
      if ((*(_work) =
	   (mat_lu_struct *) scicos_malloc (sizeof (mat_lu_struct))) == NULL)
	{
	  set_block_error (-16);
	  return;
	}
	  ...
	  NULL)
	{
      ...
      if ((ptr->IL =
	   (double *) scicos_malloc (sizeof (double) * (mu * l))) == NULL)
	{
	  set_block_error (-16);
      ...
      if ((ptr->IU =
	   (double *) scicos_malloc (sizeof (double) * (l * nu))) == NULL)
	{
	  set_block_error (-16);
	  scicos_free (ptr->IL);
	  scicos_free (ptr->dwork);
  ...
  /* Terminaison */
      ...
      for (i = 0; i < (mu * nu); i++)
	{
      ...
      C2F (zgetrf) (&mu, &nu, (doubleC *) ptr->dwork, &mu, ptr->ipiv, &info);
      if (info != 0)
	{
	  if (flag != 6)
	    {
