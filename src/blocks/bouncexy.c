...
{
  Sciprintf ("bouncexy is to be done for nsp \n");
}
...
*/
void bouncexy_draw (scicos_block * block, ScopeMemory ** pScopeMemory,
		    int firstdraw)
{
  ...
  radius_max = 0;
  size_balls =
    (double *) scicos_malloc (number_of_curves_by_subwin * sizeof (double));
  z = GetDstate (block);
  ...
  /*Creating the Scope */
  scoInitOfWindow (*pScopeMemory, dimension, win, NULL, NULL, &xmin, &xmax,
		   &ymin, &ymax, NULL, NULL);
  if (scoGetScopeActivation (*pScopeMemory) == 1)
	...
	{
	  scoAddSphereForShortDraw (*pScopeMemory, 0, j, size_balls[j],
				    colors[j]);
	}
      scoAddRectangleForLongDraw (*pScopeMemory, 0, 0, xmin,
				  (ymax - fabs (ymin)), fabs (xmax - xmin),
				  fabs (ymax - ymin));
	...
	if (scoGetScopeActivation (pScopeMemory) == 1)
	  {
	    ...
	    size_balls =
	      (double *)
	      scicos_malloc (scoGetNumberOfCurvesBySubwin (pScopeMemory, 0) *
			     sizeof (double));
	    z = GetDstate (block);
