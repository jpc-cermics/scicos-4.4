	  ...
	  switch (_ipar[count])
		...
		case 109:
		  ...
		  if (flag == 9)
			...
			{
			  _g[nzcr] =
			    (stack[bottom] - 1) * (stack[bottom] + 1);
			}
		      else if (i > 0)
			{
			  _g[nzcr] =
			    (stack[bottom] - i - 1.) * (stack[bottom] - i);
			}
		      else
			{
			  _g[nzcr] =
			    (stack[bottom] - i) * (stack[bottom] - i + 1);
			}
		      if (i % 2)
			_g[nzcr] = -_g[nzcr];
		      if (!areModesFixed (block))
		...
		case 110:
		  ...
		  if (flag == 9)
		      ...
		      else
			{
		      ...
		      _g[nzcr] =
			(stack[bottom] - i - .5) * (stack[bottom] - i + .5);
		      if (i % 2)
			_g[nzcr] = -_g[nzcr];
		      if (!areModesFixed (block))
			_mode[nzcr] = i;
		...
		case 115:
		  ...
		  if (!areModesFixed (block) || _ng == 0)
		    {
		      stack[bottom - 1] =
			max (stack[bottom - 1], stack[bottom]);
		    }
		  else
		    {
		...
		case 116:
		  ...
		  if (!areModesFixed (block) || _ng == 0)
		    {
		      stack[bottom - 1] =
			min (stack[bottom - 1], stack[bottom]);
		    }
		  else
		    {
		      stack[bottom - 1] = stack[bottom - _mode[nzcr]];
		...
		case 123:
		  stack[bottom - 1] =
		    atan2 (stack[bottom - 1], stack[bottom]);
		  bottom = bottom - 1;
		  break;
