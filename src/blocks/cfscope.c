...
*/
void cfscope_draw (scicos_block * block, ScopeMemory ** pScopeMemory,
		   int firstdraw)
{
	...
	if (scoGetScopeActivation (pScopeMemory) == 1)
	  {
	    ...
	    ipar = GetIparPtrs (block);
	    sortie =
	      (double *) scicos_malloc (nbr_of_curves * sizeof (double));
	    index_of_view =
	      (int *) scicos_malloc (nbr_of_curves * sizeof (int));
