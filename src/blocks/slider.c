...
void slider (int *flag, int *nevprt, const double *t, double *xd,
...
{
  Sciprintf ("slider to be done for nsp \n");
}
 ...
 * erase a rectangle 
 *----------------------------------------------------*/
extern int C2F (dr1) (char x0[], char x1[], int *x2, int *x3, int *x4,
		      int *x5, int *x6, int *x7, double *dx1, double *dx2,
		      double *dx3, double *dx4, int lx0, int lx1);

extern int C2F (dr) (char x0[], char x1[], int *x2, int *x3, int *x4, int *x5,
		     int *x6, int *x7, double *dx1, double *dx2, double *dx3,
		     double *dx4, int lx0, int lx1);
...
static void block_draw_rect_1 (double r[], double percent)
{
  ...
  double x = r[0];
  C2F (dr) ("xget", "alufunction", &verbose, &alumode, &narg, PI0, PI0, PI0,
	    PD0, PD0, PD0, PD0, 0L, 0L);
  if (alumode != in)
    C2F (dr1) ("xset", "alufunction", &in, PI0, PI0, PI0, PI0, PI0, PD0, PD0,
	       PD0, PD0, 0L, 0L);
  C2F (dr1) ("xfrect", "v", PI0, PI0, PI0, PI0, PI0, PI0, &x, r + 1, &w,
	     r + 3, 0L, 0L);
  if (alumode != in)
    C2F (dr1) ("xset", "alufunction", &alumode, PI0, PI0, PI0, PI0, PI0, PD0,
	       PD0, PD0, PD0, 0L, 0L);
}
...
static void block_draw_rect_3 (double r[], double percent)
{
  ...
  if (alumode != in)
    C2F (dr1) ("xset", "alufunction", &in, PI0, PI0, PI0, PI0, PI0, PD0, PD0,
	       PD0, PD0, 0L, 0L);
  sprintf (foo, "%5.3f", percent);
  C2F (dr1) ("xstring", foo, PI0, PI0, PI0, &flag, PI0, PI0, &x, &y, &ang,
	     PD0, 0L, 0L);
  if (alumode != in)
    C2F (dr1) ("xset", "alufunction", &alumode, PI0, PI0, PI0, PI0, PI0, PD0,
	       PD0, PD0, PD0, 0L, 0L);
...
static int scig_driverX11 (char *old)
{
  ...
  if (old[0] == 'R')
    {
      C2F (SetDriver) ("X11", PI0, PI0, PI0, PI0, PI0, PI0, PD0, PD0, PD0,
		       PD0);
      return 1;
    }
  return 0;
...
static int set_block_win (int win)
{
  ...
  C2F (dr) ("xget", "window", &verbose, &cur, &narg, PI0, PI0, PI0, PD0, PD0,
	    PD0, PD0, 0L, 0L);
  if (cur != win)
    C2F (dr) ("xset", "window", &win, PI0, PI0, PI0, PI0, PI0, PD0, PD0, PD0,
	      PD0, 0L, 0L);
  return cur;
}
...
{
  static int verbose = 0, cur, narg;
  C2F (dr) ("xget", "color", &verbose, &cur, &narg, PI0, PI0, PI0, PD0, PD0,
	    PD0, PD0, 0L, 0L);
  if (cur != color)
    C2F (dr) ("xset", "color", &color, PI0, PI0, PI0, PI0, PI0, PD0, PD0, PD0,
	      PD0, 0L, 0L);
  return cur;
}
	     ...
	     double *u, int *nu, double *y, int *ny)
{
  ...
  switch (*flag)
	...
	REMOVE_REC_DRIVER ();
	curcolor = set_slider_color (ipar[1]);
	C2F (dr1) ("xclea", "v", PI0, PI0, PI0, PI0, PI0, PI0, z + 2, z + 3,
		   z + 4, z + 5, 0L, 0L);
	switch (ipar[0])
	  {
	  case 1:
