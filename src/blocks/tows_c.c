...
{
  Sciprintf ("tows_c is to be done for nsp \n");
}
 ...
 */
{
  ...
  if (flag == 4)
    {				/* init */
      ...
      if ((*(_work) =
	   (towork_struct *) scicos_malloc (sizeof (towork_struct))) == NULL)
	{
	  set_block_error (-16);
	  return;
	}
       ...
       */
      if ((ptr->workt =
	   (void *) scicos_malloc (6 * sizeof (int) + 4 * sizeof (int) +
	  ...
	  if (ismat)
		  ...
		  NULL)
		{
		  ...
		  *(_work) = NULL;
	      ...
	      ptr_i = (int *) ptr->work;
	      codehm ptr_i[40] = 1;	/*head of entries for hm */
	      ptr_i[41] = nu * nu2 * nz;
	      ptr_i[42] = 1;
	      ptr_i[43] = 0;
	    ...
	    {
	      if ((ptr->work =
		   (void *) scicos_malloc (6 * sizeof (int) +
					   4 * sizeof (int) +
					   nz * nu * sizeof (double))) ==
		  NULL)
		{
	  ...
	  if (ismat)
		  ...
		  == NULL)
		{
		  ...
		  *(_work) = NULL;
	      ...
	      ptr_i = (int *) ptr->work;
	      codehm ptr_i[40] = 8;	/*head of entries for hm */
	      ptr_i[41] = nu * nu2 * nz;
	      ptr_i[42] = 1;
	      ptr_i[43] = 1;
      ...
      if (ptr != NULL)
	{
	  ...
	  /* sort data */
			  ...
			  for (j = (nz - 1); j >= 1; j--)
			      ...
			      for (k = 0; k < nu * nu2; k++)
				{
				  ...
				  *((double *) (&ptr_i[44]) + k +
				    j * (nu * nu2) + nz * nu * nu2) =
				    *((double *) (&ptr_i[44]) + k +
				      (j - 1) * (nu * nu2) + nz * nu * nu2);
				}
			    }
			  for (k = 0; k < nu * nu2; k++)
			    {
			      *((double *) (&ptr_i[44]) + k) = sav_d[k];
			      *((double *) (&ptr_i[44]) + k + nz * nu * nu2) =
				sav_d[k + nu * nu2];
			    }
			}
		    }
		...
		case SCSINT8_N:
		  if ((sav_c =
		       (SCSINT8_COP *) scicos_malloc (nu * nu2 *
						      sizeof (SCSINT8_COP)))
		      == NULL)
		    {
		      set_block_error (-16);
		...
		case SCSINT16_N:
		  if ((sav_s =
		       (SCSINT16_COP *) scicos_malloc (nu * nu2 *
						       sizeof (SCSINT16_COP)))
		      == NULL)
		    {
		      set_block_error (-16);
		...
		case SCSINT32_N:
		  if ((sav_l =
		       (SCSINT32_COP *) scicos_malloc (nu * nu2 *
						       sizeof (SCSINT32_COP)))
		      == NULL)
		    {
		      set_block_error (-16);
		...
		case SCSUINT8_N:
		  if ((sav_uc =
		       (SCSUINT8_COP *) scicos_malloc (nu * nu2 *
						       sizeof (SCSUINT8_COP)))
		      == NULL)
		    {
		      set_block_error (-16);
		...
		case SCSUINT16_N:
		  if ((sav_us =
		       (SCSUINT16_COP *) scicos_malloc (nu * nu2 *
							sizeof
							(SCSUINT16_COP))) ==
		      NULL)
		    {
		...
		case SCSUINT32_N:
		  if ((sav_ul =
		       (SCSUINT32_COP *) scicos_malloc (nu * nu2 *
							sizeof
							(SCSUINT32_COP))) ==
		      NULL)
		    {
	  ...
	  if ((ptr->cnt != 0) || (ptr->loop != 0))
		...
		case SCSREAL_N:
		  if (ismat)
		    {
		      C2F (mputnc) (&fd, &ptr_i[44],
				    (j =
				     ptr_i[37] * ptr_i[38] * ptr_i[39], &j),
				    fmtd, &ierr);
		    }
		  else
		    {
		      C2F (mputnc) (&fd, &ptr_i[10],
				    (j =
				     ptr_i[7] * ptr_i[8], &j), fmtd, &ierr);
		    }
		  break;
		case SCSCOMPLEX_N:
		  if (ismat)
