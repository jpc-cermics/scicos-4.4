function [sd]=gr_menu(sd,flag,noframe)
  global('gr_objects');
  global('gr_options');
  gr_options=gr_do_options([]);// initialize 
  scsmode=%f
  ...
  gr_draw_scene([],1);
  if async then 
    seteventhandler('gr_eventhandler');
  else
    while %t then
      [btn,xc,yc,win,Cmenu]=gr_get_click(curwin);
      if Cmenu=='Quit' then break;end
      gr_eventhandler(win,xc,yc,btn);
    end
    xdel(curwin);
    sd(3)=gr_objects;
    clearglobal gr_objects;
    clearglobal gr_options;
  end
endfunction
...
function str=gr_Edit(ind,win)
  ...
  select str 
   ...
   case 'random' then 
    ...
    gr_draw(win);
   case 'Options' then 
    gr_options=gr_do_options(gr_options);
  end
endfunction
...
function sd1 =gr_poly(action,sd,pt,pt1)
  ...
  select action 
   ...
   case 'translate' then 
    // translate sd with translation vector pt 
    if gr_objects(sd)('lock first')(1) <> 0 || gr_objects(sd)('lock last')(1) <> 0 then 
...
// 
  ...
  gr_draw_scene([],1);
  // do not want to record during move 
  xset('recording',0);
  // A améliorer XXX 
  [i,x,y]=xclick();
...
function [sd1]=gr_comment(sd,del)
  sd1=[];
  if nargin<=0 then // get
...
function [sd1]=gr_clipoff(sd,del)
  sd1=[];
  if nargin<=0 then ,
    sd1=list("clipoff")
  end;
...
function gr_eventhandler(win,x,y,ibut)
...
// move object in frame 
  ...
  gr_draw_scene([lcks,ko],1);
  while wstop==0 , //move loop
    // redraw the non-moving part.
    gr_redraw_recorded()
    xset('recording',0);
    // draw block shape
    execstr(of+'(''draw'',ko);');
...
// 
  gr_def= hash_create(color=4,background=xget('white'),foreground=6,font=2,font_size=1,clip=0);
  if nargin < 1 then   gr_options=gr_def;end 
  if  type(gr_options,'short')<>'h' then 
    gr_options=gr_def;edited=%t;return; 
  end
  colors=m2s(1:xget("lastpattern")+2,"%1.0f");
...
function [btn,xc,yc,win,Cmenu]=gr_get_click(curwin,flag)
...
function gr_draw_scene(excluded,mode)
// draw objects except the excluded ones 
// the drawing is recorded or not according to mode.
  global('gr_objects');
  others=1:length(gr_objects);
  others(excluded)=[]
  ...
  for i=others
    o=gr_objects(i);
    execstr('gr_'+o.type+'(''draw'',i);');
  end
  xset('recording',xtape_status);
endfunction

function gr_redraw_recorded()
