...
//translate scicos data structure to new version
  ...
  if version=='scicos2.7' then 
    printf('This is an old scicos diagram. I have to translate. Be patient.\n')
...
function scs_m=do_version251(scs_m)
  ...
  for k=1:nx
	...
	scs_m(k)(2)(9)=['xset(''font'',ipar(1),ipar(2))';
			'str='' ''+part(''0'',ones_new(1,ipar(5)-ipar(6)-2))+''.''+part(''0'',ones_new(1,ipar(6)))'
...
//A SUPPRIMER CF ci dessus
  if scs_m.type =='diagram' then scs_m_new=scs_m,return,end
  if scs_m(1).type <>'params' then scs_m_new=scs_m,return,end
  scs_m_new=scicos_diagram()
