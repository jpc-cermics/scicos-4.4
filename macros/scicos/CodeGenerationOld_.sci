...
function Code=old_c_make_doit1(cpr,stalone)
...
//les pointeurs de cpr :
  ...
  Code=['';
	 '/*'+part('-',ones_new(1,40))+' ddoit1.c */ ';
	 'int '
	 cformatline(rdnom+'ddoit1(double  *z,double  * told,double  * outtb,int * iwa)',70); 
	 '{'; 
	 '  int kiwa; ';
	 '  kiwa = 0; '];
  //////////////////////////////////////////////////
  if  szclkIN>=1 then
    ...
    Code($+1,1)='  pointi='+string(pointi)+'+ totalnevprt;';
   else
     if (size(cap,'*') >1) then
       pointi=clkptr(howclk)-1;
       Code($+1,1)='  pointi='+string(pointi)+'+ totalnevprt;';
...
//nouveau
function Code=old_make_cdoit(stalone)
//les pointeurs de cpr :
  z=cpr.state.z;
  pointi=cpr.state.pointi;
  funs=cpr.sim.funs;
  ...
  nZ=size(z,'*')+size(outtb,'*')+clkptr($)
  Code=['';
	'/*'+part('-',ones_new(1,40))+' cdoit.c */ ';
	'int ';
	cformatline(rdnom+'cdoit(double  *z,double  * told,double  * outtb)',70); 
	'{'; 
	'  int i2, ntvecm; '; 
	'  int flag,nport,sz[100],ierr1,nevprt,ntvec;';
...
function Code=old_make_edoit1(stalone)
  //les pointeurs de cpr :
  z=cpr.state.z;  
  funs=cpr.sim.funs;
  inpptr=cpr.sim.inpptr;
  ...
  Code=[ '/*'+part('-',ones_new(1,40))+' edoit1.c */ ';
	 'int '
	 cformatline(rdnom+'edoit1(double  *told, '+..
		    'double  *outtb,int * iwa,int * kiwa)',70)
	'{ ';
	'  int i2; ';
	'  int flag, kever, nport, nord, ierr1, ntvecm, nevprt; ';
...
function Code=old_make_doit(stalone)
    ///////////////////
  //les pointeurs de cpr :
  z=cpr.state.z;  
  funs=cpr.sim.funs;
  ...
  maxkeve=size(ordptr,1)-1;
  Code=[ '/*'+part('-',ones_new(1,40))+' doit.c */ ';
	 'int '
	 cformatline(rdnom+'doit(double  *z,double  * told,double  * outtb)',70)
	'{ ';
	'  int i2; ';
	'  int flag, kever, nport; ';
	'  int nord; ';
...
//nouveau
function Code=old_make_odoit(stalone)
//les pointeurs de cpr :
  z=cpr.state.z;
  pointi=cpr.state.pointi;
  funs=cpr.sim.funs;
  ...
  nZ=size(z,'*')+size(outtb,'*')+clkptr($)
  Code=['';
	'/*'+part('-',ones_new(1,40))+' odoit.c */ ']

  if (stalone &  ~isempty(x)) then
    Code=[Code
	  'int '
	  cformatline(rdnom+'odoit(double  *told,double *outtb,int * iwa,int  phase,double  *residual,double  * xt,double  * xtd)',70);  
	  '{']
  else
    Code=[Code
	  'int '
...
function Code=old_make_zdoit(stalone)
//les pointeurs de cpr :
  ...
  nZ=size(z,'*')+size(outtb,'*')+clkptr($)
  Code=['';
	 '/*'+part('-',ones_new(1,40))+' zdoit.c */ ';
	 'int '
	cformatline(rdnom+'zdoit(double *g,double *told,double * outtb,int * iwa,int phase)',70);  
	 '{'; 
	 '  int i,i2; '; 
	 '  int flag, keve, kiwa, nport, jj, j; ';
	 '  double  tvec['+string(sztvec)+']; '; 
	 '  double  rdouttb['+string(size(outtb,1)+1)+']; ';
	 '  double  *args[100]; '; 
	 '  int sz[100]; ';
...
function Code=old_make_ozdoit(stalone)
///////////////////
//les pointeurs de cpr :
  z=cpr.state.z;  
  funs=cpr.sim.funs;
  ...
  Code=[ '/*'+part('-',ones_new(1,40))+' ozdoit.c */ ';
        'int '
	cformatline(rdnom+'ozdoit(double  *told,double  * outtb,int * iwa,int * kiwa,int  phase)',70)	 
	'{ '; 
	'  int i2; ';
	'  int flag, kever, nport; ';
	'  int nord; ';
...
function Code=old_c_make_doit2(cpr,stalone);
...
//les pointeurs de cpr :
  ...
  nZ=size(z,'*')+size(outtb,'*')+clkptr($)
  Code=['';
	'/*'+part('-',ones_new(1,40))+' ddoit2 */ ';
	'int '
	cformatline(rdnom+'ddoit2(double  *z,double  * told,double  * outtb,int * iwa)',70);  
	'{ '; 
	'  int  i1, i; '; 
	'  int flag, keve, kiwa, nport; '; 
	'  double  *args[100]; ';
...
function Code=old_c_make_endi(cpr,stalone)
//Copyright INRIA
//Author : Rachid Djenidi
///////////////////
//les pointeurs de cpr :
  ...
  Code=[	'/*'+part('-',ones_new(1,40))+' endi */ '; 
		'/* file_end.c */ ';
		'/* Subroutine */ int ';
		rdnom+'_endi(double  *told,double  * outtb,int * iwa) ';  
		'{ '; 
		'  int flag; '; 
		'  double  rdouttb['+string(size(outtb,1)+1)+']; ';
		'  double  *args[100]; '; 
...
function Code=old_c_make_initi(cpr,stalone)
//Copyright INRIA
//Author : Rachid Djenidi
...
//les pointeurs de cpr :
  ...
  Code=['/*'+part('-',ones_new(1,40))+' initi */ ';
	'int '
	rdnom+'_initi(double  *told,double  *outtb,int * iwa) ';  
	'{ '; 
...
function Code=old_c_make_outtb(stalone)
//Copyright INRIA
//Author : Rachid Djenidi
...
//les pointeurs de cpr :
  ...
  Code=['/*'+part('-',ones_new(1,40))+' outtbini */ ';
	'int '
	rdnom+'_outtb(double  *told,double  * outtb,int * iwa) ';  
	'{ '; 
...
function [Code,actt,proto]=old_call_actuator(i)
  ...
  if funtyp(i)==0 then
    if nin==0 then
      uk=0;
      nuk=0;
      Code($+1,1)=+'  args[0]=&(outtb[0]);';
    else
      lprt=inplnk(inpptr(i));
      uk=lnkptr(lprt);
      nuk=(lnkptr(lprt+1))-uk;
      Code($+1,1)='  args[0]=&(outtb['+string(uk-1)+']);';
    end
  end
  ...
  Code($+1,1)='  block_'+rdnom+'['+string(i-1)+'].nevprt=nevprt;'
  Code=[Code;
	 '  nport = '+string(nbact)+';';
...
function [Code,proto]=old_call_block(i)
 //Copyright INRIA
//Author : Rachid Djenidi
...
function [Code,proto]=old_callf(i)
...
//Author : Rachid Djenidi
  ...
  if ftyp==0 then
	...
	uk=lnkptr(lprt);
	nuk=(lnkptr(lprt+1))-uk;
	Code($+1,1)='rdouttb['+string(k-1)+']=outtb['+string(uk-1)+'];';
      end
      Code($+1,1)='args[0]=&(rdouttb[0]);';
    elseif nin==0
      uk=0;
      nuk=0;
      Code($+1,1)=+'args[0]=&(outtb[0]);';
	...
	Code($+1,1)='rdouttb['+string(k+nin-1)+']=outtb['+..
	    string(yk-1)+'];';
      end
      Code($+1,1)='args[1]=&(rdouttb['+string(nin)+']);';
    elseif nout==0
      yk=0;
      nyk=0;
      Code($+1,1)=+'args[1]=&(outtb[0]);';
    else
      lprt=outlnk(outptr(i));
      yk=lnkptr(lprt);
      nyk=(lnkptr(lprt+1))-yk;
      Code($+1,1)='args[1]=&(outtb['+string(yk-1)+']);';
    end
  elseif ftyp==1
    if nin>=1 then
      for k=1:nin
  ...
  if ftyp==0 | ftyp==1 then
    if (cpr.sim.funtyp(i)> 2000 & cpr.sim.funtyp(i)< 3000)
      CodeC=fun+'(&flag,&block_'+rdnom+'['+string(i-1)+'].nevprt,told,block_'+rdnom+...
	    '['+string(i-1)+'].xd,block_'+rdnom+'['+string(i-1)+'].x,&block_'+rdnom+...
	    '['+string(i-1)+'].nx,block_'+rdnom+'['+string(i-1)+'].z,&block_'+rdnom+...
	    '['+string(i-1)+'].nz,block_'+rdnom+'['+string(i-1)+'].evout, &block_'+rdnom+...
	    '['+string(i-1)+'].nevout,block_'+rdnom+'['+string(i-1)+'].rpar,&block_'+rdnom+...
	    '['+string(i-1)+'].nrpar,block_'+rdnom+'['+string(i-1)+'].ipar,&block_'+rdnom+...
	    '['+string(i-1)+'].nipar';
  ...
  elseif ftyp==2 
    if ~ztyp(i) then
      CodeC=fun+'(&flag,&block_'+rdnom+'['+string(i-1)+'].nevprt,told,block_'+rdnom+...
	    '['+string(i-1)+'].xd, block_'+rdnom+'['+string(i-1)+'].x,&block_'+rdnom+...
	    '['+string(i-1)+'].nx';
      CodeC=CodeC+',block_'+rdnom+'['+string(i-1)+'].z,&block_'+rdnom+'['+string(i-1)+'].nz,block_'+...
	    rdnom+'['+string(i-1)+'].evout,&block_'+rdnom+'['+string(i-1)+'].nevout,block_'+rdnom+...
	    '['+string(i-1)+'].rpar,&block_'+rdnom+'['+string(i-1)+'].nrpar, block_'+rdnom+...
	    '['+string(i-1)+'].ipar';
      CodeC=CodeC+',&block_'+rdnom+'['+string(i-1)+'].nipar,block_'+rdnom+'['+string(i-1)+'].inptr,block_'+...
	    rdnom+'['+string(i-1)+'].insz';
      CodeC=CodeC+',&block_'+rdnom+'['+string(i-1)+'].nin,block_'+rdnom+'['+string(i-1)+'].outptr,block_'+...
	    rdnom+'['+string(i-1)+'].outsz';
	...
	CodeC=fun+'(&flag,&block_'+rdnom+'['+string(i-1)+'].nevprt,told,block_'+rdnom+...
	      '['+string(i-1)+'].xd, block_'+rdnom+'['+string(i-1)+'].x,&block_'+rdnom+...
	      '['+string(i-1)+'].nx';
	CodeC=CodeC+',block_'+rdnom+'['+string(i-1)+'].z,&block_'+rdnom+'['+string(i-1)+'].nz,block_'+rdnom+...
	      '['+string(i-1)+'].evout,&block_'+rdnom+'['+string(i-1)+'].nevout,block_'+rdnom+...
	      '['+string(i-1)+'].rpar,&block_'+rdnom+'['+string(i-1)+'].nrpar,block_'+rdnom+'['+string(i-1)+'].ipar';
	CodeC=CodeC+',&block_'+rdnom+'['+string(i-1)+'].nipar,block_'+rdnom+...
	      '['+string(i-1)+'].inptr,block_'+rdnom+'['+string(i-1)+'].insz,&block_'+rdnom+...
	      '['+string(i-1)+'].nin,block_'+rdnom+'['+string(i-1)+'].outptr,block_'+rdnom+...
	      '['+string(i-1)+'].outsz,&block_'+rdnom+'['+string(i-1)+'].nout,block_'+rdnom+...
	      '['+string(i-1)+'].g,&block_'+rdnom+'['+string(i-1)+'].ng);';
	proto='void '+fun+"(int *, int *,double *,"+...
	      " double *, double *, int *, double *, int *, double *, "+...
	      " int *, double *, int *, int *, int *, "+...
	      " double **, int *, int *, double **,int"+...
...
function [Code,capt,proto]=old_call_sensor(i)
...
function t1=old_cformatline(t ,l)
...
//  Author : Rachid Djenidi
  ...
  bl=part(' ',ones_new(1,nw))
  l1=l-nw;first=%t

  while %t 
    if length(t)<=l then t1=[t1;bl+t],return,end
    k=strindex(t,sep);
    if isempty(k) then t1=[t1;bl+t],return,end
...
function  [ok,XX,alreadyran]=old_do_compile_superblock(XX,all_scs_m,numk,alreadyran)
...
// Copyright INRIA
  ...
  elseif isempty(szclkIN)  then
    ...
    output=ones_new((2^(size(cap,'*')))-1,1)
  ...
  if cpr.equal[list()] then ok=%f,return, end
  ...
  for i=1:length(funs)-1
    ...
    if ~isempty(msg) then message(msg),ok=%f,return,end
  end
 ...
 //end
  ...
  while %t do
    ...
    //if %t then 
      // XXXX jpc waiting for enabling the code below
      //rpat="/tmp/scicos";
      //printf("FIXME: using %s\n",rpat);
      //break;
    //else
	...
	//ok=%f;x_message(rpat+' is not a directory');
      ...
      if ~file("exists",rpat) then
        ok=execstr("file(""mkdir"",rpat)",errcatch=%t)
	if ~ok then 
	  x_message('Directory '+rpat+' cannot be created');
	end
...
//  end
  ...
  if maxtotal< length(cpr.state.outtb) then
    maxtotal=length(cpr.state.outtb)
  end
...
function vec=old_codebinaire(v,szclkIN)
  vec=zeros_new(1,szclkIN)
  for i=1:szclkIN
    w=v/2;
    vec(i)=v-2*int(w);
    v=int(w);
...
function XX=old_update_block(XX)
...
function [CCode,FCode]=old_gen_blocks()
...
//Author : Rachid Djenidi
  ...
  ok=execstr('scicos_mputl(Code,rpat+''/''+rdnom+''.c'')',errcatch=%t)
  if ~ok then
    message(lasterror())
    return
  end
  if ~isempty(FCode) then
    ok=execstr('scicos_mputl(FCode,rpat+''/''+rdnom+''f.f'')',errcatch=%t)
    if ~ok then
      message(lasterror())
      return
    end
  end
  if ~isempty(CCode) then
    ok =execstr('scicos_mputl(CCode,rpat+''/''+rdnom+''_Cblocks.c'')',errcatch=%t)
    if ~ok then
      message(lasterror())
      return
    end
  ...
  Code=['/*---------------------------------------- Actuators */ '
	...
	'  int k;'
	'}']
  ok=execstr('scicos_mputl(Code,rpat+''/''+rdnom+''_void_io.c'')',errcatch=%t)
  if ~ok then
    message(lasterror())
    return
  end
  ...
  Code=[make_decl_standalone()
	...
	make_static_standalone()
        make_standalone()]
  ok=execstr('scicos_mputl(Code,rpat+''/''+rdnom+''_standalone.c'')',errcatch=%t)
  if ~ok then
    message(lasterror())
    return
  end
  ...
  created=[];
  reponse=[];  
  //XXX created=fileinfo(rpat+'/'+rdnom+'_act_sens_events.c')
  ...
  if reponse==1 |  isempty(reponse) then
    ok=execstr('scicos_mputl(Code,rpat+''/''+rdnom+''_act_sens_events.c'')',errcatch=%t)
  end
  if ~ok then
    message(lasterror())
    return
...
//Author : Rachid Djenidi
  ...
  Code=['function [x,y,typ]='+rdnom+'_c(job,arg1,arg2)';
	...
	'  iwa=zeros_new('+string(clkptr($))+',1)';
	'  work=zeros_new('+string(nblk)+',1)';
	'  Z=[z;outtb;iwa;work]';
  ...
  for i=1:length(bllst)
   ...
   Code($+1,1)='  '+sci2exp(deput,'dep_ut',70);
   Code=[Code
	 '  model=scicos_model(sim=list('''+rdnom+''',4),in=in,out=out,..'
	 '          evtin=clkinput,state=x,dstate=Z,rpar=rpar,ipar=ipar,..'
	 '          blocktype=''c'',dep_ut=dep_ut,label='' '',nzcross=nzcross,nmode=nmode)'
	 '  gr_i=''xstringb(orig(1),orig(2),'''''+rdnom+''''',sz(1),sz(2),''''fill'''')''';
	 '  x=standard_define([2 2],model,[],gr_i=gr_i,gui=""'+rdnom+'_c"")';
	 'end'
         'endfunction'];
  //Create file
  ok=execstr('F=fopen(rpat+''/''+rdnom+''_c.sci'',mode='"w"')',errcatch=%t);
  if ~ok then
    message(lasterror())
  else
    F.put_smatrix[Code];
...
function Code=old_make_actuator(standalone)
// Generating the routine for actuators interfacing
//Copyright INRIA
//Author : Rachid Djenidi
  
  Call=['/*'+part('-',ones_new(1,40))+' Actuators */ ';
	'void ';
...
function Code=old_make_computational()
...
//Copyright INRIA
  ...
  Code=['/*'+part('-',ones_new(1,40))+' Block Computational function */ ';
  ...
  if (ng ~= 0) then
    ...
    Code($+1,1)=cformatline('    '+rdnom+'zdoit(g,&t,'+..
			  '&(z['+string(size(z,1))+']), '+..
...
// '/*     date : '+date(getdate())+' */';
  ...
  Code=['/* SCILAB Computational function  */';
	'/*     Copyright INRIA */';
	'/*     Generated by Code_Generation toolbox of Scicos with Nsp */';
	'';
	'#include <stdio.h>';
	'#include <memory.h>';
	'#include <string.h>';
...
function Code=old_make_decl_standalone()
//generates  procedure declarations
//Copyright INRIA
//Author : Rachid Djenidi
//  	getversion()+' */';
...
function Code=old_make_end()
//Copyright INRIA
//Author : Rachid Djenidi
  Code=['/*'+part('-',ones_new(1,40))+' end */ ';
	'void '
...
function Code=old_make_init()
//Copyright INRIA
//Author : Rachid Djenidi
  
  Code=['/*'+part('-',ones_new(1,40))+' init */ ';
	'void '
...
//nouveau
function Code=old_make_main0()
//generate code for flag=0 case  
//Copyright INRIA
  Code=['/*'+part('-',ones_new(1,40))+' main0 */ ';
	cformatline('void '+rdnom+'main0(scicos_block *block_'+rdnom+',double *z, double *t, int phase)',70);	
	'{'];
  
  Code($+1,1)=cformatline('  '+rdnom+'odoit(t,'+..
		    '&(z['+string(size(z,1))+']), '+..
		    '(int *)(z+'+string(size(z,1)+size(outtb,1))+'), phase);',70);    
  Code($+1,1)='} '
endfunction

function Code=old_make_main1()
//generate code for flag=1 case  
//Copyright INRIA
//Author : Rachid Djenidi
  Code=['/*'+part('-',ones_new(1,40))+' main1 */ ';
	'void '
	cformatline(rdnom+'main1(scicos_block *block_'+rdnom+',double *z, double *t)',70);
	'{'];
  Code($+1,1)=cformatline('  '+rdnom+'ddoit1(z, t, '+..
		    '&(z['+string(size(z,1))+']), '+..
		    '(int *)(z+'+string(size(z,1)+size(outtb,1))+'));',70);  
...
function Code=old_make_main2()
//generate code for flag=2 case
//Copyright INRIA
//Author : Rachid Djenidi
  Code=['/*'+part('-',ones_new(1,40))+' main2 */ ';
	'void '
...
function Code=old_make_outevents()
//generates skeleton of external world events handling function
  ...
  Code=[ '/*'+part('-',ones_new(1,40))+'  External events handling function */ ';
	 'void '
...
function Code=old_make_sensor(standalone)
// Generating the routine for sensors interfacing
//Copyright INRIA
//Author : Rachid Djenidi
  
  Call=['/*'+part('-',ones_new(1,40))+' Sensor */ ';
	'void ';
...
function Code=old_make_standalone()
...
//Les fonction zdoit, cdoit, doit sont pas utilisée pour le moment
  ...
  //scicos_mputl(txt1,rpat+'/machine.h');
  //txt2=mgetl(SCI+'/routines/scicos/scicos_block.h');
  //scicos_mputl(txt2,rpat+'/scicos_block.h');
  ...
  work=zeros_new(nblk,1)
  iwa=zeros_new(clkptr($),1),Z=[z;outtb;iwa;work]';
  Code=[ '/*Main program */'
	 ...
	 '      printf('"            3 (default value) for the Fourth-Order Runge-Kutta (RK4) Formula \n'");'
         ...
         '      break;'
         '    case ''v'':'
         '      printf('" Generated by Code_Generation toolbox of Scicos with Nsp \n'");'
         '      return 0;'
         '      break;'
         '    case ''?'':'
         '      errflg++;'
	 ...
	 '}'
	 ''
	 '/*'+part('-',ones_new(1,40))+'  External simulation function */ ';
	 'int '
	 rdnom+'_sim(tf,dt,h,solver)';
	 ' '
	 '     double tf,dt,h; ';
  ...
  if size(z,1) <> 0 then
	...
	if and(aaa+bbb~= ['INPUTPORTEVTS';'OUTPUTPORTEVTS';'EVTGEN_f']) then
	  Code($+1,1)=' ';	
	  Code($+1,1)='/* Routine name of block: '+strcat(string(cpr.sim.funs(i)));
	  Code($+1,1)=' Gui name of block: '+strcat(string(OO.gui));
	  //Code($+1,1)='/* Name block: '+strcat(string(cpr.sim.funs(i)));
	//Code($+1,1)='Object number in diagram: '+strcat(string(cpr.corinv(i)));
	Code($+1,1)='Compiled structure index: '+strcat(string(i));
  ...
  Code=[Code	
	...
	'  return 0;'
	'}'  
	'/*'+part('-',ones_new(1,40))+'  Lapack messag function */ ';
	'void'
  ...
  if ~isempty(x) then
    Code=[Code
	  'int C2F('+rdnom+'simblk)(double *t,double * xc,double * xcdot)'
	  '{ '
	  '     /* '
	  '      *  !purpose '
	  ...
	  '}'
	  '/* Euler''s Method */' 
	  'int ode1(int (*f)(),double t,double h)'
	  '{'
	  '  int i;'
	  '  (*f)(&t,x, xd);'
	  '  for (i=0;i<neq;i++){'
	  ...
	  '}'
	  ' '
	  'int ode2(int (*f)(),double t,double h)'
	  '{'
	  '  /* Heun''s Method */'
	  '  int i;'
	  ...
	  '  return 0;'
	  '}'
	  'int ode4(int (*f)(),double t,double h)'
	  '{'
	  '  /* Fourth-Order Runge-Kutta (RK4) Formula */'
	  '  int i;'
	  '  double y['+string(nX)+'],yh['+string(nX)+'],temp,f0['+...
	  string(nX)+'],th,th2,f1['+string(nX)+'],f2['+string(nX)+'];'
	  '  memcpy(y,x,neq*sizeof(double));'
	  '  memcpy(f0,xd,neq*sizeof(double));'
	  '  (*f)(&t,y, f0);'
	  '  for (i=0;i<neq;i++) x[i]=y[i]+h*f0[i];'
...
function Code=old_cg_sa_static()
...
//Copyright INRIA
  ...
  Code= ['/* Table of constant values */ ';
	 ...
	 ' ';
	 'static int totalnevprt; '];
  
  if size(evtspt,1) <> 0 then
    Code=[Code;cformatline('static int evtspt[ ]={'+..
			   strcat(string(evtspt),",")+'};',70)] 
  else
    Code($+1,1)='static int evtspt[1];';
  end
  
  Code($+1,1)='static double zero=0;';
    
  if size(clkptr,1) <> 0 then
    Code=[Code;cformatline('static int clkptr[ ]={'+..
			   strcat(string(clkptr),",")+'};',70)] 
  else
    Code($+1,1)='static int clkptr[1];';
  end

  if size(ordptr,1) <> 0 then
    Code=[Code;cformatline('static int ordptr[ ]={'+..
			   strcat(string(ordptr),",")+'};',70)] 
  else
    Code($+1,1)='static int ordptr[1];';
  end  
  ...
  if size(tevts,1) <> 0 then
    ...
    Code($+1,1)='static double tevts[1];';
  end

  if size(pointi,1) <> 0 then
    Code($+1,1)='static int pointi={'+strcat(string(pointi),",")+'};'; 
  else
    Code($+1,1)='static int pointi=1;';
  end
  Code($+1,1)='static double w[1];'
  Code($+1,1)='double *'+rdnom+'_block_outtb;'
  Code($+1,1)='static int *'+rdnom+'_iwa;'
  Code($+1,1)='static int aaa=0, bbb=0;'
  Code=[Code;
	'static void set_nevprt(int nevprt)'
	'{'
	'  totalnevprt=nevprt;'
...
function Code=old_make_static()
  Code=cg_sa_static();
  Code($+1,1)='scicos_block *block_'+rdnom+';'   
endfunction

function Code=old_make_static_standalone()
//generates  static table definitions
//Copyright INRIA
//Author : Rachid Djenidi
  rpptr=cpr.sim.rpptr;ipptr=cpr.sim.ipptr;
  ...
  if size(rpar,1) <> 0 then
    ...
    Code($+1,1)='static double RPAR1[1];';
  end
  
  if size(ipar,1) <> 0 then
    
    Code=[Code;'static int IPAR1[ ] = {'];		
    ...
    Code($+1,1)='};';
    //Code=[Code;cformatline('static int IPAR1[ ]= {'+..
    //		   strcat(string(ipar),",")+'};',70)] 
  else
    Code($+1,1)='static int IPAR1[1];';
  end
endfunction
...
function Code=old_make_putevs()
//RN
  Code=['int '+rdnom+'_putevs(double *t,int * evtnb,int * ierr)';
	'{'
	'  *ierr = 0;'
	'  if (evtspt[*evtnb-1] != -1) {'
	'    *ierr = 1;'
