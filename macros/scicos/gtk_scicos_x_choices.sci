...
function []=entrykeymouse(wid,event)
...
function [entries]=GetFrameEntry(Li,vbox)
  ...
  function str_out=adjust_str_label(str_in,width_label)

    if utf8_text_length(str_in)<=width_label then
      str_out=str_in
    else
      j=1
      str_out=""
      for i=1:utf8_text_length(str_in)
        if i>j*width_label then
          //Ouchh!! pause
          ind=strindex(str_out," ")
          if ~isempty(ind) then
            str_out=utf8_part(str_out,1:ind($)-1)+'\n'+utf8_part(str_out,ind($)+1:utf8_text_length(str_out))+utf8_part(str_in,i)
          else
            str_out=str_out+'\n'+utf8_part(str_in,i)
          end
          j=j+1
        else
          str_out=str_out+utf8_part(str_in,i)
        end
      end
    end
  endfunction
  ...
  for i=1:n
    ...
    if Li(i)(1)=='entry' then
      ...
      entry.connect["key-release-event",entrykeymouse]
      entry.connect["key-press-event",entrykeymouse]
...
function L=gtk_scicos_x_choices(desc,Li)
  ...
  window.set_focus[entries(1)]
  entries(1).set_position[1]
  entries(1).set_position[entries(1).get_text_length[]]
  ...
  while ~ok
    ...
    elseif response==-4 then
      printf("DESTROY\n",response);
      if modified then
        printf("Warning modified. ASKUSER\n");
        num=0
        while num==0
