...
//Author : Rachid Djenidi
  ...
  ok=execstr('libn=ilib_compile(''lib''+rdnom,Makename)',errcatch=%t)
  if ~ok then 
    x_message(['sorry compiling problem';lasterror()]);
    return;
  end	
  // XXX libn=pathconvert(libn,%f,%t)
  ok=execstr('libnumber=link(libn)',errcatch=%t)
  if ~ok then 
    x_message(['sorry link problem';lasterror()]);
    return;
  end
  ok=execstr('link(libnumber,rdnom,''c'')',errcatch=%t)
  if ~ok then 
    x_message(['sorry link problem';lasterror()]);
    return;
  end
...
//into Scilab.
  ...
  SCode=['//exec file used to load the ""compiled"" block into Scilab'
	 ...
	 'link(fileso,rdnom,''c'')';
	 '//load the gui function';
	 'exec(DIR+''/''+rdnom+''_c.sci'');']
  
  ok=execstr('F=fopen(rpat+''/''+rdnom+''_loader.sci'',mode='"w"')',errcatch=%t);
  if ~ok then
    message(lasterror())
  else
    F.put_smatrix[SCode];
...
function Makename=gen_make(name,files,libs,Makename)
  if getenv('WIN32','NO')=='OK' then
    ...
    Makename=gen_make_unix(name,files,libs,Makename)
  end
endfunction
...
function Makename=gen_make_unix(name,files,libs,Makename)
  ...
  T=["# generated by builder.sce: Please do not edit this file"
     ...
     "CFLAGS = $(CC_OPTIONS) -I$(SCIDIR)/include/"
     "FFLAGS = $(FC_OPTIONS) -I$(SCIDIR)/include/" 
