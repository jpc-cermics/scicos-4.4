function ok=hilite_mult_objs(path,objs,mess)
  ...
  elseif type(path,"short")=="l" then
	...
	// be sure that we do not try to hilite an internal block of a csuper or asuper 
	...
	for j=1:length(temppath)
	  // we have to care care here also not to enter csuper or asuper.
	  temppathj=check_csuper(temppath(j))
	  nbr_obj=temppathj($)
	  temppath2=temppathj(1:$-1);
	  append_command("%diagram_path_objective="+sci2exp(temppath2)+";%scicos_navig=1",...
			 "hilite_obj("+sci2exp(nbr_obj)+");");
	end
      else
