...
//    model.sim=list('shift_ia',4)
	...
	elseif nb<0
	   select np
		case 0 then 
			select Datatype
			    case 3 then model.sim=list('shift_32_RA',4)
			    case 6 then model.sim=list('shift_u32_RA',4)
			end
	...
	elseif nb<0
	   select np
		case 0 then 
			select Datatype
			    case 4 then model.sim=list('shift_16_RA',4)
			    case 7 then model.sim=list('shift_u16_RA',4)
			end
	...
	elseif nb<0
	   select np
		case 0 then 
			select Datatype
			    case 5 then model.sim=list('shift_8_RA',4)
			    case 8 then model.sim=list('shift_u8_RA',4)
			end
...
gr_i=['xstringb(orig(1),orig(2),[OPER;''   Shift  ''],sz(1),sz(2),''fill'')']
  x=standard_define([3 2],model, exprs,gr_i,'SHIFT');
end
endfunction
