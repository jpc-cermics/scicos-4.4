function [x,y,typ]=ValveDiscrete(job,arg1,arg2)
  ...
  function blk_draw(sz,orig,orient,label)
    ...
    xarcs([orig(1)+3*sz(1)/10;orig(2)+sz(2);4*sz(1)/10;6*sz(2)/10;0;180* ...
	   64],scs_color(1));
    xstringb(orig(1),orig(2)+sz(2)/2,"D.",sz(1)/3,sz(2)/3,"fill")
  ...
  select job
   ...
   case "set" then
    ...
    model=arg1.model;
    while %t do
      [ok,sthreshold,price,mass,LeadTime,exprs]=getvalue('Set Valve Discrete block parameter',..
						  ['signal threshold [1]';'price [k€]';'mass [T]';'Lead Time [Day]'], ...
						  list('vec',1,'vec',1,'vec',1,'vec',1),exprs)
      if ~ok then break, end
      model.rpar=[sthreshold;price;mass;LeadTime]
      model.equations.parameters(2)=list(sthreshold,price,mass,LeadTime)
   ...
   case "define" then
    ...
    model.dep_ut=[%t %f]
    mo=modelica()
    mo.model='HydroValveDiscrete'
    mo.inputs=['p';'signal']
    mo.outputs='n'
    mo.parameters=list(['sthreshold';'price';'mass';'LeadTime'],list(sthreshold,price,mass,LeadTime))
    model.equations=mo
    
    model.in=ones(size(mo.inputs,'*'),1)
    model.out=ones(size(mo.outputs,'*'),1)
    
    exprs=[string(sthreshold);string(price);string(mass);string(LeadTime)]
    // exprs=[string(qmax);string(K);string(timemax)]
    gr_i=['txt=[''Valve Discrete''];';
	  'xstringb(orig(1),orig(2),txt,sz(1),sz(2),''fill'')']
    gr_i="blk_draw(sz,orig,orient,model.label)";
    x=standard_define([2 2],model,exprs,list(gr_i,0),'ValveDiscrete');
