...
// Copyright INRIA
  ...
  function blk_draw(sz,orig,orient,label)
    ...
    xsegs(orig(1)+sz(1)*[1/2 1/2],orig(2)+sz(2)*[0.9/8 -1/8],style=0);
  ...
  function sensor_draw_ports(o)
    ...
    if orient then  //standard orientation
      ...
      dy=sz(2)/2
      xfpoly(in2(:,1)+orig(1),in2(:,2)+(orig(2)+sz(2)-dy),1)	
    ...
    xpoly(out(:,1)+ones(4,1)*(orig(1)+dx),..
	  out(:,2)+ones(4,1)*(orig(2)-sz(2)*2/8),type="lines",close=%t)
  endfunction 
  ...
  select job
   ...
   case 'define' then
    ...
    x=standard_define([2 2],model,exprs,list(gr_i,0),'CurrentSensor');
