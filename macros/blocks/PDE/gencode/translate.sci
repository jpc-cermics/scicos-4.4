function [flag_type,rdnom,DF_type,Code]=translate(CI,CI1,CLa_type,CLa_exp,CLb_type,CLb_exp,oper,..
  ...
  if (type_meth == 2) then
    ...
    printf('Le temps de discrétisation par éléments finis est '+string(timer())+'s\n');
    // FDM
  elseif (type_meth == 1) then
    ...
    printf('Le temps de discrétisation par différences finies est '+string(timer())+'s\n');
    // FVM
  else 
    // maillage
    ...
    printf('Le temps de discrétisation par volumes finis est '+string(timer())+'s\n');
  end
  // Assemblage de code
  if (flag_type == 1) then
  ...
  printf('Le temps de la génération, compilation et linkage du code est '+string(timer())+'s\n');
