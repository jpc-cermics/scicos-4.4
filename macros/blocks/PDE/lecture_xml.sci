  ...
  ind=strindex(txt_xml,'<Node');
    ...
    if ~isempty(ind) then
      ...
      if (condi) then
        ...
        if isempty(strindex(node_attr,'/>')) then
      ...
      if ~isempty(suite ) then
        if ~isempty(strindex(suite,'<Node')) then
...
// développé par EADS-CCR
  ...
  while (i<size(ind_dollar,'*') & ind_fin ==-1) then
    ...
    if isempty(strindex(ligne,'/>')) then
      //Si c'est la fin d'un noeud pere
      if ~ isempty(strindex(ligne,'</Node>$')) then
