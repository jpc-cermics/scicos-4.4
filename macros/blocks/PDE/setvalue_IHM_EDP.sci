function [ok,a_domaine,b_domaine,discr,signe,choix,type_meth,degre,Nbr_maillage,..
  ...
  // evaluation du context non utilisée 
  // on passe le contexte a evstr
  ...
  choixman=evstr(params_pde.rad_manuel,%scicos_context);
  if (choixman) then
    // Choix (0 : Automatique, 1 : Manuel)
    choix =1;
  end
  signe=params_pde.signe;
  if (evstr(params_pde.discr_non_cst,%scicos_context)) then
    discr=1; 
  end
