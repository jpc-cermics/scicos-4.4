  ...
  select job
   ...
   case 'set' then
    ...
    while %t do
      ...
      if and(w_typ<>[0,1]) then 
	message('While type must be zero or one');ok=%f;
      elseif and(ext_out<>[0,1]) then 
	message('Show Iteration Variable must be zero or one');ok=%f;
      elseif and(iter_dtype<>[1,3,4,5]) then 
	message('Data type must be one three four five');ok=%f;
      elseif ((nbre_iter<-1)|(nbre_iter<>floor(nbre_iter))) then 
	message(['Number of Iteration must be:';'  * an integer greater than 0';' * -1']);ok=%f;
      elseif and(resetstate<>[0,1]) then 
	message('State when starting must be zero or one');ok=%f; 
      end
      if ok then
	out=ones(ext_out,2);
