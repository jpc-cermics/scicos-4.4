...
case 'set' then
  ...
  while %t do
    ...
    if satur<>0 then
      satur=1;
      if Datatype==1 then
         if size(maxp,'*')==1 then maxp=maxp*ones_deprecated(x0),end
         if size(lowp,'*')==1 then lowp=lowp*ones_deprecated(x0),end
      ...
      elseif (Datatype==2) then
         if size(maxp,'*')==1 then maxp=maxp*ones_deprecated(x0)+%i*(maxp*ones_deprecated(x0)),end
         if size(lowp,'*')==1 then lowp=lowp*ones_deprecated(x0)+%i*(lowp*ones_deprecated(x0)),end
         if (size(x0)<>size(maxp) | size(x0)<>size(lowp)) then
             message('x0 and Upper limit and Lower limit must have same size')
             ok=%f
