function [x,y,typ]=MB_S2MOn(job,arg1,arg2)
  ...
  select job
    ...
    case 'define' then
      ...
      blk=standard_define([2 2],model,[],gr_i,"MB_S2MOn");
      // adapt to Modelica 
...
function scsm= MB_S2MOn_define(n)
  ...
  // now a mux
  blk_mux = DEMUX('define');
  blk_mux.model.out= sign(n)* ones(max(n,1),1);
  blk_mux.model.outtyp= ones(max(n,1),1)
  if n > 0 then 
    blk_mux.model.in= n ;
  else
    blk_mux.model.in= -2 ;
  end
  blk_mux.model.intyp= 1;
  gr_i="blk_draw(sz,orig,orient,model.label)";
  blk_mux =standard_define([10 40],blk_mux.model,string(max(n,1)),gr_i,'MUX')
  blk_mux = set_block_origin (blk_mux, [50,10]);
  scsm.objs(2) =blk_mux;

  scsm.objs(3)=blk_s2mo;
