...
// Copyright INRIA Oct 2006
  ...
  select job
   ...
   case 'set' then
	...
	// this is dangerous because in a context where getvalue 
	// is not interactive we will loop indefinitely.
	...
	[model,graphics,ok]=set_io(model,graphics,list([ones_deprecated(in),ones_deprecated(in)],ones_deprecated(in)),..
				   list([ones_deprecated(out),ones_deprecated(out)],ones_deprecated(out)),..
				   [],[],intypex,outtypex)
      end
      //============================
	...
	exprs.paramv=list();
	if Tparam_sz<>0 then //if param
	  if type(lab_2,'short')=='l' then //for eval
	    for i=1:length(lab_2)
