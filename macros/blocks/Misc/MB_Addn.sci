function [x,y,typ]=MB_Addn(job,arg1,arg2)
  ...
  function txt = MB_Addn_funtxt(H, dim_r, signs)
    // n : signal dimensions
  ...
  function blk= MB_Addn_define(dim_r, signs,old)
    if nargin <= 2 then 
      global(modelica_count=0);
      nameF='addn'+string(modelica_count);
    ...
    H.funtxt = MB_Addn_funtxt(H, dim_r, signs);
